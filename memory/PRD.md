...
- **CRITICAL signup 500 fix — stale unique index** (2026-06): Live server signup returned HTTP 500 and no account created. Root cause (from deployed docker logs): a legacy **unique index `email_1` on `studioapp.admins`** rejected every new admin because they have `email: null` (Mongo allows only one null in a unique index). Diagnosed by probing the live server (studioappgallery.uk) directly + reading their Dockge backend logs. FIX: added `startup_fix_indexes()` on_event startup that auto-drops non-sparse unique indexes on admins/tenants `email`/`contact_email` (self-heals on rebuild — no manual DB work). Also wrapped `self_signup` in try/except that logs full traceback + returns real error detail instead of blank 500. VERIFIED: creating the exact index then running the routine drops it; signup+login 200/200 after.
