...
- **Mobile signup/login "invalid credentials" fix** (2026-06): Root cause = mobile keyboards auto-capitalize the username field, so signup saved e.g. "John" but login typed "john" → mismatch. FIX: (1) login lookup is now case-insensitive + whitespace-trimmed (`control_db.admins.find_one` uses `^{re.escape(username.strip())}$` case-insensitive regex); (2) signup trims username + case-insensitive uniqueness; (3) frontend username inputs (Signup.js + AdminLogin.js) now have autoCapitalize="none" autoCorrect="off" autoComplete="username" spellCheck=false. Added `import re` at module top. VERIFIED: signup "MixedCase", login "mixedcase" + trailing spaces all succeed. Frontend Signup.js also shows friendly "please sign in" + redirect on username-already-exists.
