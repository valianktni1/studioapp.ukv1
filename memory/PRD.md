...
- **Order-paid email to photographer** (2026-06): `notify_tenant_order_paid(order)` emails the tenant (their SMTP smtp_email, branded via build_branded_email) with item breakdown, delivery, total & customer email whenever a print order transitions to "paid". `notify_client_order_receipt(order)` also emails the CLIENT (customer_email) a branded receipt/confirmation. Both triggered from `paypal_capture_order` (API flow, on COMPLETED) and legacy `update_order_paypal` PUT (guarded against duplicate). Best-effort (threaded send, skips silently if tenant SMTP unconfigured). VERIFIED end-to-end via seeded order.
- **Tenant logo in email footer** (2026-06): `build_branded_email(..., logo_url="")` renders the tenant's logo above the footer text. `get_tenant_logo_email_url(smtp)` resolves the tenant doc's logo_url to an absolute URL using their SMTP site_url (skips if no logo/site_url). Wired into all tenant emails: order-paid, client receipt, gallery-ready notify, template send, and 7-day expiry reminders. Email settings tab shows an inline hint (site-url-logo-hint) prompting to set Gallery Site URL so the logo appears. VERIFIED.
- **Signup bug fix** (2026-06): "Account created but submit showed an error." Root cause: `_provision_tenant` inserted tenant+admin, then did a filesystem `mkdir` for the demo gallery — on self-hosted storage this could raise AFTER the account existed, returning 500. FIX: wrapped default-template + demo-gallery/mkdir in try/except (best-effort, logged) so signup always returns token even if demo/fs step fails. VERIFIED via simulated fs failure.
- **Mobile signup/login "invalid credentials" fix** (2026-06): mobile keyboards auto-capitalize username → signup saved "John", login typed "john". FIX: login lookup case-insensitive + trimmed regex; signup trims + case-insensitive uniqueness; frontend username inputs autoCapitalize=none/autoCorrect=off/spellCheck=false. Added `import re` module-top.
- **CRITICAL signup 500 fix — stale unique index** (2026-06): legacy unique index `email_1` on admins rejected new accounts (email null). FIX: `startup_fix_indexes()` auto-drops non-sparse unique email/contact_email indexes on admins/tenants; signup wrapped in try/except surfacing real error. VERIFIED 200/200.
- **Mobile dashboard stats layout** (2026-06): 5 stat cards changed from fixed grid-cols-5 to `grid-cols-1 sm:grid-cols-2 lg:grid-cols-5`; on phones cards stack (label-left/value-right) instead of overlapping. Desktop unchanged.
- **Backlog batch (2026-06):**
  - **Mobile top-nav** — AdminDashboard header nav now `hidden md:flex`; on mobile a "New" button + hamburger (`mobile-menu-btn`) opens a stacked dropdown (`mobile-menu-panel`) with all nav items. Additive; desktop unchanged.
  - **PayPal webhook** — `POST /api/webhook/paypal/{tenant_id}`: extracts PayPal order id from payload, re-verifies status via tenant's PayPal API creds (no signature trust needed), marks order paid + notifies both parties. Idempotent (skips already-paid).
  - **Password reset (forgot password)** — `POST /api/admin/forgot-password` (username or email, generic response, no enumeration) emails a 1-hour single-use reset link via tenant SMTP (sha256-hashed token in control_db.password_resets); `POST /api/admin/reset-password` (token + new bcrypt password). Frontend: AdminPasswordReset.js page (routes /admin/forgot & /admin/reset), "Forgot your password?" link on AdminLogin. VERIFIED end-to-end: reset works, login with new pw, token reuse blocked, mark pw restored.
  - **Favourites-only ZIP** and **2FA (TOTP)** — confirmed ALREADY implemented (endpoints + UI); no work needed.

## SUPER ADMIN: STORAGE & USAGE (2026-06)
- New **"Storage & Usage"** tab in SuperAdmin.js (between Photographers and Payments).
- **Storage** — `GET /api/super/storage`: per-tenant MongoDB `dbStats.storageSize` + on-disk media size (`_dir_size_bytes` walks UPLOAD_DIR/<tid>, run in executor). Returns total_bytes + per_tenant [db_bytes, disk_bytes, total_bytes], sorted desc. Frontend `fmtBytes()` auto-switches KB→MB→GB→TB (TB when ≥1024GB). Big platform figure + per-photographer breakdown.
- **Bandwidth** — control_db `usage_stats` collection {tenant_id, date(YYYY-MM-DD), upload_bytes, download_bytes} via `record_usage(tenant_id, kind, nbytes)` (best-effort $inc upsert). Hooks: uploads (`upload_files`, `guest_upload`), downloads/streaming (`stream_video_direct` via `_tenant_from_upload_path`, `stream_share_video`, `download_share_file`, `download_share_zip`, album, favourites, admin download-file/subfolder). Range requests record served `length`; full responses record file size; ZIPs sum file_size. `GET /api/super/bandwidth`: Today + last-7-days upload/download totals + per-tenant week. History builds from deploy. NOTE: when NGINX_VIDEO_URL is set, video streaming bypasses Python so those bytes aren't counted (downloads/uploads still tracked).
- VERIFIED via curl: upload recorded 50000B, download 8228B; storage totals per tenant; UI screenshot confirmed (GB/TB auto-format, per-tenant lists, up/down split).

## DEFERRED (P1, confirmed with user 2026-06)
- Large-video FFMPEG faststart is still SYNCHRONOUS in `generate_thumbnails_background` (line ~1297, `ensure_video_faststart`). For 5GB+ videos it blocks a thumbnail worker. FIX: decouple faststart to `transcode_executor` (async worker). NOT YET DONE — user deferred to after the dashboard.
