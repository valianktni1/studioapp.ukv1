...
- **Video optimiser size threshold + upload guidance** (2026-06): (1) Added constants VIDEO_OPTIMISE_MIN_BYTES=300MB, VIDEO_OPTIMISE_MAX_BYTES=20GB. `generate_thumbnails_background` now only submits `create_web_version` when 300MB<=size<=20GB; smaller/larger videos stream the original (faststart still applied to all; stream falls back to original at server.py ~2742). (2) AdminGalleryDetail.js video-optimise-note now includes bold "Recommended: upload 1 video at a time..." guidance. VERIFIED note renders + backend healthy.
