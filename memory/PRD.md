...
- **Video optimisation badge** (2026-06): Gallery detail (`get_gallery_detail`) now annotates video files with `has_web` (web-optimised copy exists on disk) + `optimise_eligible` (300MB–20GB). AdminGalleryDetail.js shows a persistent corner badge per video: green "✓ Optimised" (has_web), amber spinning "Optimising…" (eligible, not yet done), none for out-of-range videos. Transcode polling now calls loadGallery() on completion so the badge flips to Optimised automatically. VERIFIED via seeded test videos (Optimised / Optimising… / no-badge for <300MB); test data cleaned up.
